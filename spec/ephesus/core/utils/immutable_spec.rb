...
RSpec.describe Ephesus::Core::Utils::Immutable do
  ...
  describe '::from_array' do
    it { expect(described_class).to respond_to(:from_array).with(1).argument }

    describe 'with nil' do
      it 'should raise an error' do
        expect { described_class.from_array nil }
          .to raise_error ArgumentError, 'argument must be an Array'
      end
    end

    describe 'with an empty array' do
      it { expect(described_class.from_array []).to be_a Hamster::Vector }

      it { expect(described_class.from_array []).to be_empty }
    end

    describe 'with an array with items' do
      let(:ary)       { %w[one two three] }
      let(:immutable) { described_class.from_array ary }
  ...
  describe '::from_hash' do
    ...
    describe 'with an empty hash' do
      it { expect(described_class.from_hash({})).to be_a Hamster::Hash }

      it { expect(described_class.from_hash({})).to be_empty }
    end
    ...
    describe 'with a hash with string keys' do
      ...
      let(:immutable) { described_class.from_hash hsh }
  ...
  describe '::from_set' do
    it { expect(described_class).to respond_to(:from_set).with(1).argument }

    describe 'with nil' do
      it 'should raise an error' do
        expect { described_class.from_set nil }
          .to raise_error ArgumentError, 'argument must be a Set'
      end
    end

    describe 'with an empty set' do
      it { expect(described_class.from_set Set.new).to be_a Hamster::Set }

      it { expect(described_class.from_set Set.new).to be_empty }
    end

    describe 'with a set with items' do
      let(:set)       { Set.new(%w[one two three]) }
      let(:immutable) { described_class.from_set set }
  ...
  describe '::to_array' do
    it { expect(described_class).to respond_to(:to_array).with(1).argument }

    describe 'with nil' do
      it 'should raise an error' do
        expect { described_class.to_array nil }
          .to raise_error ArgumentError, 'argument must be a Hamster::Vector'
      end
    end

    describe 'with an empty vector' do
      let(:immutable) { Hamster::Vector.new }
      let(:ary)       { described_class.to_array immutable }
  ...
  describe '::to_hash' do
    it { expect(described_class).to respond_to(:to_hash).with(1).argument }

    describe 'with nil' do
      it 'should raise an error' do
        expect { described_class.to_hash nil }
          .to raise_error ArgumentError, 'argument must be a Hamster::Hash'
      end
    end

    describe 'with an empty hash' do
      let(:immutable) { Hamster::Hash.new }
      let(:hsh)       { described_class.to_hash immutable }

      it { expect(hsh).to be_a Hash }

      it { expect(hsh).to be_empty }
    end
  ...
  describe '::to_set' do
    it { expect(described_class).to respond_to(:to_set).with(1).argument }

    describe 'with nil' do
      it 'should raise an error' do
        expect { described_class.to_set nil }
          .to raise_error ArgumentError, 'argument must be a Hamster::Set'
      end
    end

    describe 'with an empty set' do
      let(:immutable) { Hamster::Set.new }
      let(:set)       { described_class.to_set immutable }
    ...
    describe 'with a set of data objects' do
      ...
      let(:immutable) { Hamster::Set.new(immutable_data[:spells]) }
      let(:set)       { described_class.to_set immutable }
